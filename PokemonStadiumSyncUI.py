...
def load_config():
    ...
    try:
        with open(CONFIG_FILE, "r", encoding="utf-8") as file:
            ...
            config["DolphinPort2"] = re.search(r'DolphinPort2 = "(.*?)"', content).group(1)
            config["DolphinPort3"] = re.search(r'DolphinPort3 = "(.*?)"', content).group(1)
            config["DolphinPort4"] = re.search(r'DolphinPort4 = "(.*?)"', content).group(1)
...
dropdown_options = {
    ...
    "DolphinPort2": ["", "Ruby", "Sapphire", "Emerald", "FireRed", "LeafGreen"],
    "DolphinPort3": ["", "Ruby", "Sapphire", "Emerald", "FireRed", "LeafGreen"],
    "DolphinPort4": ["", "Ruby", "Sapphire", "Emerald", "FireRed", "LeafGreen"],
}

for idx, (key, value) in enumerate(data.items()):
    tk.Label(root, text=key).grid(row=idx, column=0, padx=10, pady=5, sticky="w")
    
    if key in dropdown_options:
        var = tk.StringVar()
        var.set(value)
        dropdown = tk.OptionMenu(root, var, *dropdown_options[key])
        dropdown.config(width=12)  # Set dropdown width
        ...
        if key in ["base_dir", "gb_dir", "gba_dir", "sav_dir"]:
            browse_button = tk.Button(root, text="Browse", command=lambda e=entry, k=key: browse_folder(e, k))
        elif key in ["Stadium 1", "Stadium 2"]:
            browse_button = tk.Button(root, text="Browse", command=lambda e=entry: browse_file(e, [("N64 ROMs", "*.n64;*.z64")]))
        elif key in ["Green", "Red", "Blue", "Yellow", "Gold", "Silver", "Crystal"]:
            browse_button = tk.Button(root, text="Browse", command=lambda e=entry: browse_file(e, [("GB Saves", "*.srm;*.gb;*.gbc")], strip_extension=True))
        elif key in ["Ruby", "Sapphire", "Emerald", "FireRed", "LeafGreen"]:
            browse_button = tk.Button(root, text="Browse", command=lambda e=entry: browse_file(e, [("GBA Saves", "*.srm;*.gba")], strip_extension=True))
        else:
            browse_button = None
